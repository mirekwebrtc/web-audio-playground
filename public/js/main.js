...
function createAudioBufferSource( buffer ) {
	...
	sel.appendChild( opt );
	opt = document.createElement("option");
	opt.appendChild( document.createTextNode("wagner-short.ogg"));
	sel.appendChild( opt );
...
function startLoadingSounds() {
	...
	noiseRequest.open("GET", "sounds/wagner-short.ogg", true);
	...
	guitarRequest.send();
        ...
        wagnershortRequest = new XMLHttpRequest();
        wagnershortRequest.open("GET", "sounds/wagner-short.ogg", true);
        wagnershortRequest.responseType = "arraybuffer";
        wagnershortRequest.onload = function() {
          audioContext.decodeAudioData( wagnershortRequest.response, function(buffer) {
                wagnershortBuffer = buffer;
                buffers[6]= wagnershortBuffer;
                } );
        }
        wagnershortRequest.send();
